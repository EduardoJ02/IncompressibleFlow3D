#ifndef COMPUTAH_H_INCLUDED
#define COMPUTAH_H_INCLUDED

#define A u3D[i+1][j][k]
#define B u3D[i][j][k]
#define C u3D[i-1][j][k]
#define D u3D[i][j+1][k]
#define E u3D[i][j-1][k]

#define a v3D[i+1][j][k]
#define b v3D[i][j][k]
#define e v3D[i][j-1][k]
#define q v3D[i+1][j-1][k]

#define Batras u3D[i][j][k-1]
#define Bb w3D[i][j][k]
#define Aai w3D[i+1][j][k]
#define Cc w3D[i][j][k-1]
#define Qqi w3D[i+1][j][k-1]

#define Aa w3D[i][j][k+1]
#define Cc w3D[i][j][k-1]

#define Aaj w3D[i][j+1][k]
#define Cci w3D[i-1][j][k]
#define Ccj w3D[i][j-1][k]

#define Bf u3D[i][j][k+1]
#define bf v3D[i][j][k+1]

#define Cf u3D[i-1][j][k+1]
#define ef v3D[i][j-1][k+1]
